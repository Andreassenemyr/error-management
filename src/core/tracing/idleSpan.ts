...
import { Span, SpanAttributes } from "../../node/span/types";
import { logger } from "../../utils/logger";
import { SEMANTIC_ATTRIBUTE_RIBBAN_IDLE_SPAN_FINISH_REASON } from "../semanticAttributes";
import { _setSpanForScope } from "./spanOnScope";
import { getActiveSpan, getSpanDescendants, removeChildSpanFromSpan, spanTimeInputToSeconds, spanToJSON } from "./spanUtilts";
...
export function startIdleSpan(startSpanOptions: StartSpanOptions, options: Partial<IdleSpanOptions> = {}): Span {
    ...
    function onIdleSpanEnded(endTimestamp: number): void {
        ...
        if (!attributes[SEMANTIC_ATTRIBUTE_RIBBAN_IDLE_SPAN_FINISH_REASON]) {
            span.setAttribute(SEMANTIC_ATTRIBUTE_RIBBAN_IDLE_SPAN_FINISH_REASON, _finishReason);
        }
